  ...
  <div v-if="open" class="history-page">
    <section class="history-content">
      ...
      <template v-else>
        ...
        <section class="history-section"><h4>个性化营养建议</h4><template v-if="detail.personalized_analysis"><p class="advice-summary">{{ detail.personalized_analysis.summary }}</p><ul v-if="detail.personalized_analysis.suggestions?.length"><li v-for="item in detail.personalized_analysis.suggestions" :key="item">{{ item }}</li></ul><p v-if="detail.personalized_analysis.follow_up"><strong>随访建议：</strong>{{ detail.personalized_analysis.follow_up }}</p></template><p v-else class="muted">本次归档时尚未生成个性化营养建议。</p></section>
      </template>
    </section>
  </div>
</template>
