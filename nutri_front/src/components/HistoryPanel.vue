...
const editingExportInfo = ref(false)
const exportInfoSaving = ref(false)
const exportInfo = ref({})
const exportInfoError = ref('')
const exportInfoExcluded = ref(false)
const exportFields = [
  ['A', '登记号/住院号'], ['B', '患者姓名'], ['C', '会诊医生'], ['D', 'NRS2002总分'],
  ['E', 'MNA-SF总分'], ['F', '筛查日期'], ['G', 'MNA评分'], ['H', 'PG-SGA'],
  ['I', 'SGA分级'], ['J', '评估日期'], ['K', 'GLIM分期结果'], ['L', '诊断日期'],
]
...
async function editExportInfo(id) {
  await view(id)
  if (detail.value?.id === id) await startEditExportInfo()
}

async function startEditExportInfo() {
  exportInfoError.value = ''
  try {
    const response = await fetch(`${API_BASE}/history/${detail.value.id}/export-info`)
    const data = await response.json().catch(() => ({}))
    if (!response.ok) throw new Error(data.detail || '导出信息读取失败。')
    exportInfo.value = { ...data.values }
    exportInfoExcluded.value = Boolean(data.excluded)
    editingExportInfo.value = true
  } catch (err) {
    error.value = err.message || '导出信息读取失败。'
  }
}
async function saveExportInfo() {
  if (exportInfoSaving.value) return
  exportInfoSaving.value = true
  exportInfoError.value = ''
  try {
    const response = await fetch(`${API_BASE}/history/${detail.value.id}/export-info`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ values: exportInfo.value }),
    })
    const data = await response.json().catch(() => ({}))
    if (!response.ok) throw new Error(data.detail || '导出信息保存失败。')
    exportInfo.value = { ...data.values }
    exportInfoExcluded.value = false
    editingExportInfo.value = false
  } catch (err) {
    exportInfoError.value = err.message || '导出信息保存失败。'
  } finally {
    exportInfoSaving.value = false
  }
}
function cancelEditExportInfo() { editingExportInfo.value = false; exportInfoError.value = '' }
async function removeFromExport() {
  if (!window.confirm('确认仅从导出信息表中删除该行吗？历史记录、报告和图片都会保留。')) return
  try {
    const response = await fetch(`${API_BASE}/history/${detail.value.id}/export-info`, { method: 'DELETE' })
    const data = await response.json().catch(() => ({}))
    if (!response.ok) throw new Error(data.detail || '从导出表删除失败。')
    exportInfoExcluded.value = Boolean(data.excluded)
  } catch (err) {
    exportInfoError.value = err.message || '从导出表删除失败。'
  }
}
...
async function downloadExportInfo() {
  try {
    const response = await fetch(`${API_BASE}/history/export.xlsx?download_at=${Date.now()}`, { cache: 'no-store' })
    if (!response.ok) throw new Error('导出信息表下载失败。')
    const url = URL.createObjectURL(await response.blob())
    const link = document.createElement('a')
    link.href = url
    link.download = '导出信息.xlsx'
    link.click()
    window.setTimeout(() => URL.revokeObjectURL(url), 1000)
  } catch (err) {
    error.value = err.message || '导出信息表下载失败。'
  }
}
...
<template>
  <div v-if="open" class="history-page">
    <section class="history-content">
      <header class="history-header"><div><p>History archive</p><h2>历史记录</h2></div><div class="history-header-actions"><button class="secondary-button" type="button" @click="downloadExportInfo">↓ 导出信息表</button><button class="secondary-button" type="button" @click="emit('close')">返回筛查</button></div></header>
      ...
      <template v-else>
        <div class="history-actions"><button class="secondary-button" type="button" @click="detail = null">← 返回列表</button><button class="secondary-button" type="button" @click="startEditExportInfo">编辑导出信息</button><button class="danger-button" type="button" @click="remove(detail.id)">删除此记录</button></div>
        <h3 class="record-title">{{ detail.created_at }}_{{ detail.patient_name }}</h3>
        <section v-if="editingExportInfo" class="history-section export-info-editor"><div class="report-section-header"><h4>编辑导出信息</h4><span class="muted">仅影响下载的导出信息表，不修改原始评估、DOCX、图片或历史报告。</span></div><p v-if="exportInfoExcluded" class="export-excluded-note">该记录已从导出信息表中删除；保存编辑信息即可恢复纳入导出。</p><div class="export-info-grid"><label v-for="[key, label] in exportFields" :key="key"><span>{{ key }} · {{ label }}</span><input v-model="exportInfo[key]" type="text" /></label></div><p v-if="exportInfoError" class="error-alert">{{ exportInfoError }}</p><div class="history-actions export-info-actions"><button v-if="!exportInfoExcluded" class="danger-button" type="button" @click="removeFromExport">从导出表删除</button><button class="secondary-button" type="button" :disabled="exportInfoSaving" @click="cancelEditExportInfo">取消</button><button class="primary-button" type="button" :disabled="exportInfoSaving" @click="saveExportInfo">{{ exportInfoSaving ? '保存中...' : exportInfoExcluded ? '恢复纳入导出' : '保存导出信息' }}</button></div></section>
...
.history-list-row{display:grid;grid-template-columns:16px minmax(0,1fr) auto;align-items:center;gap:12px;margin:10px 0;padding:0;border:1px solid #dceaf2;border-radius:9px;background:#fff;overflow:hidden;transition:.18s}.history-list-row:hover{border-color:#b9dce9}.history-list-row.selected{border-color:#83c7d8;background:#f0fbfd}.history-list-row .history-item{width:100%;min-height:62px;margin:0!important;padding:16px;border:0!important;border-radius:0;background:transparent}.history-export-edit{min-height:34px;margin-right:12px;padding:0 10px;border:1px solid #9ecbd8;border-radius:7px;background:#f4fbfd;color:#147a92;font:inherit;font-size:13px;font-weight:800;white-space:nowrap}.history-export-edit:hover{background:#e4f4f8}.history-select{display:flex;position:static;align-items:center;justify-content:center;width:16px;height:16px;margin-left:14px;border:0;background:transparent;cursor:pointer}.history-select input{position:static;width:16px;height:16px;margin:0;opacity:1;accent-color:#147a92}.history-select span{display:none}
</style>
