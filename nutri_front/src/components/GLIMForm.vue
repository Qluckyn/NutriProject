...
function restoreFromDraft() {
  ...
  reducedIntake.value = saved.reducedIntake ?? 'false'
  inflammationBurden.value = saved.inflammationBurden ?? 'false'
...
function persistForm() {
  ...
  draftContext.draftData.glim_form = {
    ...
    reducedIntake: reducedIntake.value,
    inflammationBurden: inflammationBurden.value,
...
watch([muscleLoss, reducedIntake, inflammationBurden, selectedDiseaseIds, crp, il6], persistForm, { deep: true })
...
<template>
  <section class="scale-form">
    <div class="form-card">
      ...
      <div class="disease-section"><h3>疾病勾选列表</h3><div v-for="group in groupedDiseases" :key="group.type" class="disease-group"><h4>{{ group.title }}</h4><label v-for="item in group.items" :key="item.id" class="check-option"><input v-model="selectedDiseaseIds" type="checkbox" :value="item.id" /><span>{{ item.name }}</span></label></div></div>
      <div class="form-grid two-cols"><label class="field-block"><span>CRP</span><input v-model="crp" type="number" min="0" step="0.1" placeholder="选填，如有检测结果请填写" /><small>mg/L</small></label><label class="field-block"><span>IL-6</span><input v-model="il6" type="number" min="0" step="0.1" placeholder="选填，如有检测结果请填写" /><small>pg/mL</small></label></div>
    ...
    <section v-if="result" class="assessment-result" :class="result.is_malnourished ? 'risk' : 'good'"><div class="score-hero"><span>诊断结果</span><strong>{{ result.is_malnourished ? '营养不良' : '未诊断营养不良' }}</strong><em v-if="result.severity" :class="result.severity.includes('重度') ? 'severity-red' : 'severity-orange'">{{ result.severity }}</em></div><div class="criteria-columns"><div><h3>表型标准</h3><span v-for="item in result.phenotypic_criteria_triggered" :key="item" class="tag-pill">{{ item }}</span><p v-if="!result.phenotypic_criteria_triggered.length">未触发</p></div><div><h3>病因标准</h3><span v-for="item in result.etiological_criteria_triggered" :key="item" class="tag-pill">{{ item }}</span><p v-if="!result.etiological_criteria_triggered.length">未触发</p></div></div><div class="result-metrics three"><div><span>6个月内体重丢失</span><strong>{{ result.weight_loss_6m_pct }}%</strong></div><div><span>6个月以上体重丢失</span><strong>{{ result.weight_loss_over6m_pct }}%</strong></div><div><span>BMI</span><strong>{{ result.bmi }}</strong></div></div><p class="message-text">{{ result.message }}</p><button class="secondary-button" type="button" @click="resetResult">重新评估</button></section>
  </section>
</template>
