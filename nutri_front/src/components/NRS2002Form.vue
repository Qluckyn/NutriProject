  ...
  <section class="scale-form">
    <div class="form-card">
      <div class="section-title-row">
        <div>
          <p class="section-kicker">NRS-2002</p>
          <h2>营养风险筛查</h2>
        </div>
      </div>
      ...
      <div class="field-block">
        <span>全身情况是否受损</span>
        <small>因疾病导致一般状态差，如虚弱、卧床等</small>
        <div class="segmented-options">
          <label><input v-model="generalConditionImpaired" type="radio" value="true" />是</label>
          <label><input v-model="generalConditionImpaired" type="radio" value="false" />否</label>
        </div>
      </div>

      <div class="disease-section">
        <h3>疾病严重程度</h3>
        <div v-for="group in groupedDiseases" :key="group.score" class="disease-group">
          <h4>{{ group.score }}分疾病</h4>
      ...
      <button class="primary-button" type="button" :disabled="loading || !formReady" @click="submit">
        <span v-if="loading" class="spinner" aria-hidden="true"></span>
        {{ loading ? '评估中...' : '开始评估 - NRS-2002' }}
      </button>
    ...
    <section v-if="result" class="assessment-result" :class="result.has_risk ? 'risk' : 'good'">
      <div class="score-hero">
        ...
        <em>{{ result.risk_level }}</em>
      </div>
      <div class="result-metrics three">
        <div><span>营养状态受损</span><strong>{{ result.nutrition_score }}分</strong></div>
        <div><span>疾病严重程度</span><strong>{{ result.disease_score }}分</strong></div>
        <div><span>年龄</span><strong>{{ result.age_score }}分</strong></div>
      </div>
      <div class="result-metrics four">
        <div><span>1个月丢失</span><strong>{{ result.weight_loss_details['1m_loss_pct'] }}%</strong></div>
        <div><span>2个月丢失</span><strong>{{ result.weight_loss_details['2m_loss_pct'] }}%</strong></div>
        <div><span>3个月丢失</span><strong>{{ result.weight_loss_details['3m_loss_pct'] }}%</strong></div>
        <div><span>BMI</span><strong>{{ result.bmi }}</strong></div>
      </div>
      <p class="message-text">{{ result.recommendation }}</p>
      <p class="message-text">{{ result.message }}</p>
      <button class="secondary-button" type="button" @click="resetResult">重新评估</button>
    </section>
  </section>
</template>
