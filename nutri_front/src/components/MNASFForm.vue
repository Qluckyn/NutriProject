...
async function submit() {
  ...
  result.value = null
  try {
    const response = await fetch(`${API_BASE}/assess/mna_sf`, {
      method: 'POST',
  ...
  <section class="scale-form">
    <div class="form-card">
      <div class="section-title-row">
        <div>
          <p class="section-kicker">MNA-SF</p>
          <h2>微型营养评估简表</h2>
        </div>
      </div>
    ...
    <section v-if="result" class="assessment-result" :class="levelClass(result.level)">
      ...
      <div class="result-metrics three"><div v-for="(score, key) in result.score_breakdown" :key="key"><span>{{ key }}</span><strong>{{ score }}分</strong></div><div><span>{{ useBmi === 'true' ? 'BMI' : '小腿围' }}</span><strong>{{ useBmi === 'true' ? result.bmi : calfValue }}</strong></div><div><span>3个月体重下降</span><strong>{{ result.weight_loss_3m_kg }}kg</strong></div></div>
