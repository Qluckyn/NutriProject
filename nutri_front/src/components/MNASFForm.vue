...
const isValidCalf = computed(() => {
  const numeric = Number(calfCircumference.value)
  return calfCircumference.value !== '' && calfCircumference.value !== null && calfCircumference.value !== undefined && Number.isFinite(numeric) && numeric > 0
})
const formReady = computed(() => (
  ...
  && (calfCircumference.value === '' || isValidCalf.value)
))
...
function restoreFromDraft() {
  ...
  calfCircumference.value = saved.calfCircumference ?? saved.calfOverride ?? ''
...
function persistForm() {
  ...
  draftContext.draftData.mnasf_form = {
    ...
    calfCircumference: calfCircumference.value,
  }
...
function buildPayload() {
  ...
  return {
    ...
    calf_circumference: calfCircumference.value === '' ? null : Number(calfCircumference.value),
  }
}
...
async function submit() {
  ...
  if (!formReady.value) {
    ...
    errorMessage.value = '请填写有效的年龄、身高、当前体重和最近一周摄食量；如填写小腿围，需为大于0的数值。'
    return
  }
  loading.value = true
  errorMessage.value = ''
...
watch([mobility, stressOrAcuteDisease, mentalStatus, calfCircumference], persistForm)
...
<template>
  <section class="scale-form">
    <div class="form-card">
      ...
      <label class="field-block"><span>小腿围（选填）</span><input v-model="calfCircumference" type="number" min="0.1" step="0.1" placeholder="cm" /><small>第6题会与 BMI 同时计算；两者都有时按较低分计入。</small></label>
